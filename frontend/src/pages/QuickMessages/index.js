...
const Quickemessages = () => {
  ...
  const handleDeleteQuickemessage = async (quickemessageId) => {
    try {
      ...
      toast.success(i18n.t("Atalho removido com sucesso!"));
    } catch (err) {
      toastError(err);
    }
            ...
            <TableBody>
              {quickemessages.map((quickemessage) => (
                <TableRow key={quickemessage.id} hover>
                  <TableCell>
                    <Typography variant="body2" style={{ fontWeight: 500, color: '#171717' }}>
