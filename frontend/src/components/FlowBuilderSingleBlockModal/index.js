...
const FlowBuilderSingleBlockModal = ({
  ...
  const handleElements = newNameFiles => {
    ...
    for (let i = 0; i < numberImg; i++) {
      ...
      if (elementsSeqEdit.includes(newArrImg[i])) {
        const filteredElements = elementsEdit.filter(item => item.number === newArrImg[i]);
        if (filteredElements.length > 0) {
          const itemSelectedEdit = filteredElements[0];
          elementsSequence.push({
            type: "img",
          ...
          console.error('No matching element found for the image.');
        }
      } else {
        let indexElem = 0;
    ...
    for (let i = 0; i < numberPdf; i++) {
        ...
        if (i < onlyPdfNameOriginal.length) {
            elementsSequence.push({
                type: "pdf",
                value: onlyPdf[i],
            ...
            console.error('Invalid index for onlyPdfNameOriginal');
        }
    }
  ...
  const handleSaveNode = async () => {
    ...
    if (open === "edit") {
      ...
      setTimeout(async () => {
        if (
          (numberAudio === 0 && numberVideo === 0 && numberImg === 0 && medias.filter(file => file.name.includes("pdf")).length === 0) ||
          medias.length === 0
        ) {
          try {
            const mountData = {
      ...
      setTimeout(async () => {
        if (numberAudio === 0 && numberVideo === 0 && numberImg === 0 && medias.filter(file => file.name.includes("pdf")).length === 0) {
          try {
            const mountData = {
              seq: elementsSeq,
              elements: handleElements(null)
