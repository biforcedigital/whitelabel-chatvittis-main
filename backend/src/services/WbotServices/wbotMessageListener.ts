...
export const handleMessageIntegration = async (
  ...
  // Verificar o tipo de integração e executar o processamento correspondente
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    let requestData: any = {
      msg,
      msgType,
      ticket: { id: ticket.id, uuid: ticket.uuid, isTicketCreated: isTicketCreated }
    };

    // Enviar o áudio somente se a mensagem for do tipo 'audioMessage'
    if (msgType === "audioMessage") {
      try {
        const media = await downloadMedia(msg, ticket?.imported, wbot, ticket);
        requestData.audio = media.data.toString('base64');
      } catch (error) {
    ...
    if (msgType === "imageMessage") {
      try {
        const media = await downloadMedia(msg, ticket?.imported, wbot, ticket);
        requestData.imageBase64 = media.data.toString('base64');
      } catch (error) {
    ...
    // Enviar a requisição após um pequeno delay
    setTimeout(async () => {
      ...
      try {
        request(options, (error, response) => {
          if (error) {
            ...
            console.log("Resposta da integração:", response.body);
          }
        });
      } catch (error) {
    ...
    if (msgType === "audioMessage") {
      ...
      try {
        inputAudio = await new Promise<string>((resolve, reject) => {
          readFile(
        ...
        logger.error(`Erro ao ler o arquivo de áudio: ${err}`);
      }
    }
...
const handleMessage = async (
  ...
  try {
    ...
    isTicketCreated = !existingTicket && !!ticket;
    


    // Verificar se este ticket está aguardando resposta de um nó de input
    // @ts-ignore
    if (!msg.key.fromMe && ticket?.dataWebhook?.waitingInput === true && ticket?.dataWebhook?.inputVariableName) {
      try {
    ...
    console.log("Detailed Integration Condition Check:", {
      "ticket.imported": !ticket.imported,
      "msg.key.fromMe": !msg.key.fromMe,
      "ticket.isGroup": !ticket.isGroup,
      "ticket.user": !ticket.user,
      // "isQueueAndIntegration": isQueueAndIntegration,
      "integrationId is not nil": !isNil(whatsapp.integrationId)
    });
    if (
      !ticket.imported &&
      !msg.key.fromMe &&
      ...
      const integrations = await ShowQueueIntegrationService(whatsapp.integrationId, companyId);
      await handleMessageIntegration(msg, wbot, companyId, integrations, ticket, contact, ticket, null, isTicketCreated)
