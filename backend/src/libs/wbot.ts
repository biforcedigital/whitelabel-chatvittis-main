...
export const initWASocket = async (whatsapp: Whatsapp): Promise<Session> => {
  ...
  `})
        ...
        // if (true && !wsocket.authState.creds.registered) {
        //   const phoneNumber = await question(
        //     "Insira o seu número de telefone celular: +"
        //   );
        //   if (phoneNumber) {
        //     const code = await wsocket.requestPairingCode(
        //       phoneNumber.replace(/[^0-9]/g, "")
        //     );
        //     console.log(
        //       "Se houver uma notificação do WhatsApp [Inserindo o código para vincular um novo dispositivo], então está garantido que foi bem-sucedido!"
        //     );
        //     console.log(`phoneNumber:${phoneNumber.replace(/[^0-9]/g, "")} pairing code: ${code.match(/.{1,4}/g).join("-")}`);
        //   } else {
